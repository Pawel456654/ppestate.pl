...
export default function Navbar() {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 h-24 transition-colors duration-300 ${
          menuOpen
            ...
            <div className="flex items-center gap-3">
              <div className="h-full w-16 shrink-0 flex items-center justify-center">
                <Image
                  ...
                  height={64}
                  sizes="64px"
                  className={`h-16 w-16 object-contain scale-[1.12] translate-y-[2px] transition-[filter] duration-300 ${
                    menuOpen ? "brightness-0 invert md:brightness-100 md:invert-0" : ""
                  }`}
                  preload
                />
              </div>
          ...
          >
            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path
                ...
                className={`transition-all duration-300 origin-center ${menuOpen ? "opacity-0 scale-90" : "opacity-100 scale-100"}`}
              />
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                ...
                className={`transition-all duration-300 origin-center ${menuOpen ? "opacity-100 scale-100" : "opacity-0 scale-90"}`}
              />
            </svg>
          </button>
        </div>
      ...
      {/* Mobile overlay — zawsze w DOM, płynna animacja wejścia i wyjścia */}
      <div
        className={`md:hidden fixed inset-0 z-40 bg-[#0d3479] transition-[opacity,visibility,transform] duration-300 ease-in-out ${
          menuOpen
      ...
      >
        {/* Przycisk X wewnątrz menu */}
        <button
          onClick={() => setMenuOpen(false)}
          className="absolute top-7 right-9 p-2 text-white/70 hover:text-white transition-colors duration-200"
